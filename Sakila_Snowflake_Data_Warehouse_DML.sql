...
SELECT
    ...
    f.film_key,
    st.store_key,
    COALESCE(rd.date_key, 0) AS rental_date_key,
    COALESCE(re.date_key, 0) AS return_date_key,
    1 AS count_rentals,
    1 AS count_returns,
    f.film_rental_duration AS rental_duration,
    p.amount AS dollar_amount
FROM sakila.rental r
JOIN sakila_snowflake.dim_customer c ON r.customer_id = c.customer_id
JOIN sakila_snowflake.dim_staff s ON r.staff_id = s.staff_id
JOIN sakila.inventory i ON r.inventory_id = i.inventory_id
JOIN sakila_snowflake.dim_film f ON i.film_id = f.film_id
JOIN sakila_snowflake.dim_store st ON i.store_id = st.store_id
LEFT JOIN sakila_snowflake.dim_date rd ON DATE(r.rental_date) = rd.date
LEFT JOIN sakila_snowflake.dim_date re ON DATE(r.return_date) = re.date
JOIN sakila.payment p ON r.rental_id = p.rental_id
ON DUPLICATE KEY UPDATE
    rental_last_update = r.last_update,
    customer_key = c.customer_key,
    staff_key = s.staff_key,
    film_key = f.film_key,
    store_key = st.store_key,
    rental_date_key = COALESCE(rd.date_key, 0),
    return_date_key = COALESCE(re.date_key, 0),
    count_rentals = 1,
    count_returns = 1,
    rental_duration = f.film_rental_duration,
    dollar_amount = p.amount;
