...
VALUES ('Credit Card'), ('Cash'), ('Paypal'), ('Cheque');

# 1h) Rename table payment_type to payment_types.
RENAME TABLE sakila.payment_type TO sakila.payment_types;

# 1i) Drop the table payment_types.
DROP TABLE sakila.payment_types;
...
SELECT title, rental_rate * (12 / rental_duration) AS rental_cost
FROM sakila.film
WHERE title = 'ACADEMY DINOSAUR';

# 2h) List all the unique city IDs in the address table. 
# Note: If you want to challenge yourself, list list all the unique cities in the address table. 
...
FROM sakila.film
WHERE replacement_cost > 15 OR rental_duration BETWEEN 5 AND 10;
...
SELECT STDDEV(rental_rate) AS std_rental_rate
FROM sakila.film;
