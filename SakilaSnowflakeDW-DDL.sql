...
CREATE TABLE IF NOT EXISTS `sakila_snowflake`.`fact_rental` (
    ...
    rental_last_update TIMESTAMP NULL,
    customer_key INT(8) NOT NULL,
    staff_key INT(8) NOT NULL,
    film_key INT(8) NOT NULL,
    store_key INT(8) NOT NULL,
    rental_date_key BIGINT(20) NOT NULL,
    return_date_key BIGINT(20) NOT NULL,
    count_rentals INT(8) NOT NULL DEFAULT 1,
    count_returns INT(8) NOT NULL DEFAULT 1,
    rental_duration INT(10) NOT NULL,
    dollar_amount FLOAT NOT NULL,
    FOREIGN KEY (customer_key) REFERENCES sakila_snowflake.dim_customer(customer_key),
    FOREIGN KEY (staff_key) REFERENCES sakila_snowflake.dim_staff(staff_key),
    FOREIGN KEY (film_key) REFERENCES sakila_snowflake.dim_film(film_key),
    FOREIGN KEY (store_key) REFERENCES sakila_snowflake.dim_store(store_key)
);
